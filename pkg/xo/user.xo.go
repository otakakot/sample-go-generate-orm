...
func (u *User) Insert(ctx context.Context, db DB) error {
	...
	const sqlstr = `INSERT INTO public.users (` +
		...
		`) RETURNING id`
	// run
	logf(sqlstr, u.Name, u.CreatedAt, u.UpdatedAt)
	if err := db.QueryRowContext(ctx, sqlstr, u.Name, u.CreatedAt, u.UpdatedAt).Scan(&u.ID); err != nil {
		return logerror(err)
	}
	// set exists
	u._exists = true
...
func UserByName(ctx context.Context, db DB, name string) (*User, error) {
	...
	if err := db.QueryRowContext(ctx, sqlstr, name).Scan(&u.ID, &u.Name, &u.CreatedAt, &u.UpdatedAt); err != nil {
